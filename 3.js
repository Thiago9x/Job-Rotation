...
let valorDias = [
  ...
  for (let i = 0; i < valorDias.length; i++) {
    //maior valor
    maiorValor = Math.max(maiorValor, valorDias[i].valor);
    //menor valor
    menorValor = Math.min(menorValor, valorDias[i].valor);
    // quantidade de dias em que o valor de faturamento foi a cima da média
    somaValorDias += valorDias[i].valor;
  }
  let mediaValorDias = somaValorDias / valorDias.length;
  let valoresAcimaMedia = valorDias.filter((faturamento) => {
    return faturamento.valor > mediaValorDias;
  });
